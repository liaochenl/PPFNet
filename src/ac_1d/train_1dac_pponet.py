...
if __name__ == "__main__":
    ...
    if pre:
        ...
        u_encoder = MLP_std(u.shape[1],u_latent_dim,4)
        projection_dim = 30
        net = PUPRE(temperature, p_latent_dim, u_latent_dim,
                p_encoder, u_encoder, projection_dim, u_length=u.shape[1])
        net.cuda()
    ...
    train_epoch = 10001
    # Load data
    train_loader, val_loader, test_l_loader, test_r_loader = get_dataloader(data_type, task_type='PPONet', test_rho=test_rho, batch_size=500, num_samples=num_samples)
    ...
    model.cuda()
    # define the optimizer
    opt2 = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-5)
    lrs = torch.optim.lr_scheduler.StepLR(opt2, step_size=2000, gamma=0.5)
    print(f'Start Train PPONet: total {train_epoch} epochs')
    model.train()
    for epoch in range(train_epoch):
        loss_value = 0
        for i, (train_p, train_v, y, train_u) in enumerate(train_loader):
    ...
    print('Test Done!')
    torch.save(model.state_dict(), 'PPONet_1D_AC.pth')
    print('Model saved!')
